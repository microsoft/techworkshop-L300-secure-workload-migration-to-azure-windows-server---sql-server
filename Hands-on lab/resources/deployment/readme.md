...
- Simulated on-premises environment
  - Simulated on-premises SQL Server machine
- Azure Bastion to enable secure Remote Desktop connections to Virtual Machine
...
- The ARM template was generated by authoring the Azure Bicep script (`deploy.bicep`) first, and then using the Azure Bicep CLI `az bicep build --file deploy.bicep` command to create the matching ARM Template. Any updates required should follow the same pattern of updating the Bicep script and using the Azure Bicep CLI to push those changes into the ARM template.
