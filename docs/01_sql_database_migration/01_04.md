---
title: '4. Upgrade to latest compatibility level and configure HA/DR'
layout: default
nav_order: 4
parent: 'Exercise 01: SQL Migration to Azure via Arc MI-Link'
---

# Task 04 - Upgrade to latest compatibility level and configure HA/DR

## Introduction

With Tailspin's database workloads now running in Azure SQL Managed Instance, the next priority is to ensure long‑term resilience and modernization. This task focuses on transitioning the Managed Instance to the latest compatibility level (versionless mode) to unlock modern SQL features while maintaining backward compatibility. You will also configure high availability (HA) and disaster recovery (DR) to meet Tailspin's strict business continuity requirements. By completing this task, you will strengthen both performance and resilience of the migrated workloads.

## Description

In this task, you will:

1. Upgrade the SQL Managed Instance to the latest compatibility level, ensuring applications can take advantage of modern SQL capabilities.
2. Configure high availability (HA) to protect against local failures and maintain uptime.
3. Implement disaster recovery (DR) by enabling geo‑replication or failover groups, ensuring resilience against regional outages.
4. Validate HA/DR configurations by simulating failover scenarios and confirming application continuity.

## Success Criteria

- SQL Managed Instance is upgraded to the latest compatibility level and applications continue to function correctly.
