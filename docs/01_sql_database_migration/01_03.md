---
title: '3. Execute rollback and failback strategies'
layout: default
nav_order: 3
parent: 'Exercise 01: SQL Migration to Azure via Arc MI-Link'
---

# Task 03 - Execute rollback and failback strategies

## Introduction

Once MI‑Link is configured and replication is active, Tailspin must ensure that migration plans include resilience and reversibility. Business continuity requires not only the ability to fail over to Azure SQL Managed Instance, but also the ability to rollback or failback to the on‑premises SQL Server if needed. This task demonstrates how to safely reverse migration steps, validate data integrity, and confirm that workloads can return to the original environment without downtime or data loss.

## Description

In this task, you will:

- Perform a rollback to revert workloads back to the on‑premises SQL Server after failover.
- Validate failback by re‑establishing synchronization and ensuring the database is consistent across environments.
- Confirm business continuity by testing that applications remain available during rollback/failback operations.

## Success Criteria

- Rollback to the on‑premises SQL Server is executed successfully.
- Failback synchronization is validated and data integrity is confirmed.
- Applications remain available and business continuity is maintained throughout rollback/failback operations.
